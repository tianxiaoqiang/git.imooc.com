...
//用于对外暴露红包应用服务，应该是唯一的暴露点
var IRedEnvelopeService RedEnvelopService

func GetRedEnvelopeService() RedEnvelopService{
	base.Check(IRedEnvelopeService)
	return IRedEnvelopeService
}

//在此先不提供对外暴露点，先写要对外暴露的接口

type RedEnvelopService interface {
	//接口下提供一下方法
	//1.发红包
	SendOut(dto *RedEnvelopSendingDTO)
	//2.收红包
	Receive(dto *RedEnvelopReceiveDTO)
...
//发送红包DTO
type RedEnvelopSendingDTO struct {
...
//收红包DTO
type RedEnvelopReceiveDTO struct {
	EnvelopeNo      string `json:"EnvelopeNo" validate:"required"`
	ReceiveUserId   string `json:"ReceiveUserId" validate:"required"`
	ReceiveUserName string `json:"ReceiveUserName" validate:"required"`
